...
# @yaml
# signature: remove <filename>
# docstring: removes the specified file
# arguments:
#   filename:
#     type: string
...
# @yaml
# signature: save <filename> <json_data>
# docstring: saves the provided JSON data to the specified filename in the root directory
# arguments:
#   filename:
#     type: string
#     description: the name of the JSON file to create
#     required: true
#   json_data:
#     type: string
...
save() {
  ...
  cd ..
  if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: save <filename> <json_data>"
    return 1
  fi

  # Save JSON data to analysis/filename, assuming analysis directory exists
  echo "$2" > "analysis/$1"
  echo "JSON data saved to 'analysis/$1'."

  # Display present working directory (optional)
  echo "Current working directory: $(pwd)"

  # Check if analysis directory exists and warn if not
  if [ ! -d "analysis" ]; then
    echo "Warning: 'analysis' directory not found. Data saved to current directory."
  fi
